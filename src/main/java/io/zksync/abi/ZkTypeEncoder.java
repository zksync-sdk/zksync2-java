...
public class ZkTypeEncoder {
  ...
  static boolean isDynamic(Type parameter) {
    return parameter instanceof DynamicBytes
        ...
        || (parameter instanceof StaticArray
            && DynamicStruct.class.isAssignableFrom(((StaticArray) parameter).getComponentType()));
  }
   ...
   * after the padding or length, in arrays cases, is removed resulting the packed encode hex value
   *
   ...
   * decoded as dynamic values and have their offsets at the beginning of the encoding. Example: For
   * the following static array containing three elements: <code>StaticArray3</code> enc([struct1,
   * struct2, struct2]) = offset(enc(struct1)) offset(enc(struct2)) offset(enc(struct3))
   * enc(struct1) enc(struct2) enc(struct3)
   *
   * @param
   * @return
   */
   ...
   *     <p>An array of dynamic structs (ie, structs containing dynamic datatypes) is encoded in the
   *     following way: Considering X = [struct1, struct2] for example enc(X) = head(struct1)
  ...
  private static <T extends Type> String encodeArrayValuesOffsets(DynamicArray<T> value) {
    ...
    if (arrayOfBytes || arrayOfString) {
      ...
      for (int i = 0; i < value.getValue().size(); i++) {
        ...
        result.append(
            Numeric.toHexStringNoPrefix(
                Numeric.toBytesPadded(new BigInteger(Long.toString(offset)), MAX_BYTE_LENGTH)));
      }
