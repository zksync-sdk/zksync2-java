...
public class ContractDeployer {
  ...
  /**
   * Compute contract address according <a
   ...
   */
  public static Address computeL2Create2Address(
      Address sender, byte[] bytecode, byte[] constructor, byte[] salt) {
  ...
  public static Function encodeCreate(byte[] bytecode, byte[] calldata) {
    ...
    return new Function(
        "create",
        Arrays.asList(
            new Bytes32(new byte[32]), new Bytes32(bytecodeHash), new DynamicBytes(calldata)),
        Collections.emptyList());
  }

  /**
   * Encode `create2` deployment custom account function of default factory contract (see <a
   * href="https://eips.ethereum.org/EIPS/eip-4337">EIP-4337</a>)
   *
   * @param bytecode Compiled bytecode of the Custom Account contract
   * @return Encoded contract function
   */
  public static Function encodeCreate2Account(byte[] bytecode) {
    return encodeCreate2Account(
        bytecode, new byte[] {}, new byte[32], AccountAbstractionVersion.Version1);
  }
  ...
  public static Function encodeCreate2Account(byte[] bytecode, byte[] calldata) {
    return encodeCreate2Account(
        bytecode, calldata, new byte[32], AccountAbstractionVersion.Version1);
  }
   ...
   * @return Encoded contract function
   */
  public static Function encodeCreate2Account(
      byte[] bytecode,
      byte[] calldata,
      byte[] salt,
    ...
    return new Function(
        ...
        Arrays.asList(
            new Bytes32(salt),
            new Bytes32(bytecodeHash),
            new DynamicBytes(calldata),
            new Uint8(accountAbstractionVersion.getRawValue())),
        Collections.emptyList());
  }

  /**
   * Encode `create` deployment custom account function of default factory contract (see <a
   * href="https://eips.ethereum.org/EIPS/eip-4337">EIP-4337</a>)
   *
   * @param bytecode Compiled bytecode of the Custom Account contract
   * @return Encoded contract function
