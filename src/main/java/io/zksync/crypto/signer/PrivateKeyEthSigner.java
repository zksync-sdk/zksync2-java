...
public class PrivateKeyEthSigner implements EthSigner {
  ...
  @Override
  public <S extends Structurable> CompletableFuture<String> signTypedData(
      Eip712Domain domain, S typedData) {
    return this.signMessage(Eip712Encoder.typedDataToSignedBytes(domain, typedData), false);
  }

  @Override
  public <S extends Structurable> CompletableFuture<Boolean> verifyTypedData(
      Eip712Domain domain, S typedData, String signature) {
    return this.verifySignature(
        signature, Eip712Encoder.typedDataToSignedBytes(domain, typedData), false);
  }
  ...
  public CompletableFuture<Boolean> verifySignature(
      String signature, byte[] message, boolean prefixed) {
