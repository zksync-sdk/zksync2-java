...
public class ZkSyncWallet {
  ...
  public ZkSyncWallet(ZkSync zksync, EthSigner signer) {
    this(
        zksync,
        signer,
        new ZkSyncTransactionReceiptProcessor(
            zksync, DEFAULT_POLLING_FREQUENCY, DEFAULT_POLLING_ATTEMPTS_PER_TX_HASH),
  ...
  public RemoteCall<TransactionReceipt> transfer(
      String to, BigInteger amount, @Nullable Token token, @Nullable BigInteger nonce) {
        ...
        () -> {
          ...
          Transaction estimate =
              Transaction.createFunctionCallTransaction(
                  signer.getAddress(), txTo, BigInteger.ZERO, BigInteger.ZERO, txAmount, calldata);

          EthSendTransaction sent = estimateAndSend(estimate, nonceToUse).join();

          try {
            return this.transactionReceiptProcessor.waitForTransactionReceipt(
                sent.getTransactionHash());
  ...
  /**
   * Deploy new smart-contract into chain (this method uses create2, see <a
  ...
  public RemoteCall<TransactionReceipt> deploy(
      byte[] bytecode, @Nullable byte[] calldata, @Nullable BigInteger nonce) {
    return new RemoteCall<>(
        () -> {
          BigInteger nonceToUse = nonce != null ? nonce : getNonce().send();
  ...
  public RemoteCall<TransactionReceipt> deployAccount(byte[] bytecode, byte[] calldata) {
    return deployAccount(bytecode, calldata, null, null);
  }

  public RemoteCall<TransactionReceipt> deployAccount(
      byte[] bytecode, byte[] calldata, BigInteger nonce) {
    return deployAccount(bytecode, calldata, null, nonce);
  }

  public RemoteCall<TransactionReceipt> deployAccount(
      byte[] bytecode, byte[] calldata, byte[] salt) {
    return deployAccount(bytecode, calldata, salt, null);
  }

  public RemoteCall<TransactionReceipt> deployAccount(
      byte[] bytecode, byte[] calldata, @Nullable byte[] salt, @Nullable BigInteger nonce) {
    return new RemoteCall<>(
        () -> {
          BigInteger nonceToUse = nonce != null ? nonce : getNonce().send();
          ...
          Function function =
              ContractDeployer.encodeCreate2Account(
                  bytecode, calldata, saltToUse, AccountAbstractionVersion.Version1);
          ZkFunctionEncoder encoder = new ZkFunctionEncoder();
          String data = encoder.encodeFunction(function);
          Transaction estimate =
              Transaction.create2ContractTransaction(
                  signer.getAddress(),
                  BigInteger.ZERO,
                  BigInteger.ZERO,
                  Numeric.toHexString(bytecode),
  ...
  public RemoteCall<TransactionReceipt> execute(
      String contractAddress, Function function, @Nullable BigInteger nonce) {
    return new RemoteCall<>(
        () -> {
          BigInteger nonceToUse = nonce != null ? nonce : getNonce().send();
          String calldata = FunctionEncoder.encode(function);
  ...
  /**
   * Get balance of wallet by address in {@link Token} at block {@link DefaultBlockParameter} also
   * see {@link org.web3j.protocol.core.DefaultBlockParameterName}, {@link
   * org.web3j.protocol.core.DefaultBlockParameterNumber}, {@link ZkBlockParameterName}
   *
    ...
    if (token.isETH()) {
      ...
      ERC20 erc20 =
          ERC20.load(
              token.getL2Address(),
              this.zksync,
              new ReadonlyTransactionManager(this.zksync, getSigner().getAddress()),
  ...
  /**
   * Get nonce for wallet at block {@link DefaultBlockParameter} (wallet address gets from {@link
   * EthSigner}) also see {@link org.web3j.protocol.core.DefaultBlockParameterName}, {@link
   * org.web3j.protocol.core.DefaultBlockParameterNumber}, {@link ZkBlockParameterName}
   *
   * @param at Block variant
  ...
  public RemoteCall<BigInteger> getNonce(DefaultBlockParameter at) {
    return new RemoteCall<>(
        () ->
            this.zksync
                .ethGetTransactionCount(signer.getAddress(), at)
                .sendAsync()
                .join()
                .getTransactionCount());
  }
  ...
  /**
   * Get nonce for wallet at block `COMMITTED` {@link ZkBlockParameterName} (wallet address gets
   * from {@link EthSigner})
   *
   * @return Prepared get nonce call
   */
  ...
  public RemoteCall<TransactionReceipt> sendMessageToL1(
      byte[] message, @Nullable BigInteger nonce) {
    return new RemoteCall<>(
        () -> {
          BigInteger nonceToUse = nonce != null ? nonce : getNonce().send();
  ...
  private CompletableFuture<EthSendTransaction> estimateAndSend(
      Transaction transaction, BigInteger nonce) {
    return CompletableFuture.supplyAsync(
            () -> {
              ...
              String signature =
                  signer
                      .getDomain()
                      .thenCompose(domain -> signer.signTypedData(domain, prepared))
                      .join();
              byte[] signed =
                  TransactionEncoder.encode(
                      prepared, TransactionEncoder.getSignatureData(signature));
