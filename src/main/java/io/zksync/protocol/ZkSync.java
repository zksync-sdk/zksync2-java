...
public interface ZkSync extends Web3j {
  ...
  /**
   * Get list of the tokens supported by ZkSync. The tokens are returned in alphabetical order by
   * their symbol, so basically, the token id is its position in an alphabetically sorted array of
   * tokens.
   *
   ...
   * @param sender The sender of the message (i.e. the account that called the L1Messenger system
   *     contract)
   * @param message The keccak256 hash of the message that was sent
   * @param l2LogPosition The index in the block of the event that was emitted by the L1Messenger
   *     when submitting this message. If it is omitted, the proof for the first message with such
   *     content will be returned
   * @return Prepared get proof for message request
   */
  Request<?, ZksMessageProof> zksGetL2ToL1MsgProof(
      Integer block, String sender, String message, @Nullable Long l2LogPosition);
  ...
  /**
   * Get transaction receipt. The same as eth_getTransactionReceipt but with additional fields
   * belong to L2ToL1 messenger See {@link io.zksync.utils.Messenger}
   *
   ...
   * @param returnFullTransactionObjects If true it returns the full transaction objects, if false
   *     only the hashes of the transactions
   * @return Prepared get transaction receipt request
   */
  Request<?, ZksBlock> zksGetBlockByHash(String blockHash, boolean returnFullTransactionObjects);

  /**
   * Get block by number.
   *
   * @param defaultBlockParameter integer of a block number, or the string "earliest", "latest" or
