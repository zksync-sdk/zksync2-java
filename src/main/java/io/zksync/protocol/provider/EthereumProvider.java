...
public interface EthereumProvider {
   ...
   * @return CompletableFuture for waiting for transaction mine
   */
  CompletableFuture<BigInteger> getBaseCost(
   ...
   * @return CompletableFuture for waiting for transaction mine
   */
  CompletableFuture<TransactionReceipt> requestExecute(
  ...
  /**
   * Send deposit transaction to ZkSync contract. For ERC20 token must be approved before. @see
   * EthereumProvider.approveDepodits
   *
   * @param token - Token object supported by ZkSync
   * @param amount - Amount tokens to transfer
   * @param operatorTips - Tips for operator on L2 that executes deposit
   ...
   * @return CompletableFuture for waiting for transaction mine
   */
  CompletableFuture<TransactionReceipt> deposit(
   ...
   * @return CompletableFuture for waiting for transaction mine
   */
  CompletableFuture<TransactionReceipt> withdraw(
   ...
   */
  CompletableFuture<Boolean> isDepositApproved(
  ...
  static CompletableFuture<EthereumProvider> load(
      ZkSync zksync,
      Web3j ethereum,
    ...
    return CompletableFuture.supplyAsync(
        () -> {
          BridgeAddresses bridgeAddresses =
              zksync.zksGetBridgeContracts().sendAsync().join().getResult();
          String mainContract = zksync.zksMainContract().sendAsync().join().getResult();
          IL1Bridge erc20Bridge =
              IL1Bridge.load(
          ...
          ZkSyncContract zkSyncContract =
              ZkSyncContract.load(mainContract, ethereum, transactionManager, gasProvider);
          return new DefaultEthereumProvider(
              ethereum, zksync, transactionManager, gasProvider, zkSyncContract, erc20Bridge);
        });
  }
}
