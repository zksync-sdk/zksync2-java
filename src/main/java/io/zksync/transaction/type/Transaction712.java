...
public class Transaction712 extends Transaction1559 implements Structurable {
  ...
  public List<RlpType> asRlpValues(Sign.SignatureData signatureData) {
    Assertions.verifyPrecondition(
        signatureData != null || getMeta().getCustomSignature() != null,
        "One of `signatureData` and `meta.customSignature` MUST be set");
    ...
    if (getFactoryDeps() != null) {
      factoryDeps =
          Arrays.stream(getFactoryDeps()).map(RlpString::create).collect(Collectors.toList());
    } else {
    ...
    if (getMeta().getPaymasterParams() != null
        && getPaymaster() != null
        && getPaymasterInput() != null) {
      paymasterParams =
          Arrays.asList(
              RlpString.create(
                  Numeric.hexStringToByteArray(getMeta().getPaymasterParams().getPaymaster())),
  ...
  public String getPaymaster() {
    return getMeta() != null && getMeta().getPaymasterParams() != null
        ? getMeta().getPaymasterParams().getPaymaster()
        : null;
  }

  public byte[] getPaymasterInput() {
    return getMeta() != null && getMeta().getPaymasterParams() != null
        ? getMeta().getPaymasterParams().getPaymasterInput()
        : null;
  }
  ...
  public List<Pair<String, Type<?>>> eip712types() {
    ...
    result.add(
        Pair.of("to", getTo() != null ? new Uint256(Numeric.toBigInt(getTo())) : Uint256.DEFAULT));
    ...
    result.add(
        Pair.of(
            "paymaster",
            getPaymaster() != null
                ? new Uint256(Numeric.toBigInt(getPaymaster()))
                : Uint256.DEFAULT));
    ...
    result.add(
        Pair.of(
            "data",
            getData() != null
                ? new DynamicBytes(Numeric.hexStringToByteArray(getData()))
    ...
    result.add(
        Pair.of(
            "paymasterInput",
            getPaymasterInput() != null
                ? new DynamicBytes(getPaymasterInput())
                : DynamicBytes.DEFAULT));
