...
public class Eip712EncoderTest {
  ...
  public static void setUp() {
    ...
    domain =
        new Eip712Domain(
            "Ether Mail", "1", ZkSyncNetwork.Mainnet, "0xCcCCccccCCCCcCCCCCCcCcCccCcCCCcCcccccccC");
  }

  @Test
  public void testEncodeType() {
    final String result = Eip712Encoder.encodeType(message.intoEip712Struct());

    assertEquals(
        "Mail(Person from,Person to,string contents)Person(string name,address wallet)", result);
  }

  @Test
  public void testHashEncodedType() {
    final byte[] hash = Eip712Encoder.typeHash(message.intoEip712Struct());
  ...
  public void testEncodeContentsValue() {
    final byte[] hash = Eip712Encoder.encodeValue(message.contents).getValue();
  ...
  public void testEncodePersonData() {
    final byte[] fromHash = Eip712Encoder.encodeValue(message.from.intoEip712Struct()).getValue();
    final byte[] toHash = Eip712Encoder.encodeValue(message.to.intoEip712Struct()).getValue();
  ...
  public void testEncodeMailData() {
    final byte[] data = Eip712Encoder.encodeValue(message.intoEip712Struct()).getValue();
  ...
  public void testEncodeDomainMemberValues() {
    ...
    {
      final byte[] data = Eip712Encoder.encodeValue(domain.getVersion()).getValue();
    ...
    {
      final byte[] data = Eip712Encoder.encodeValue(domain.getChainId()).getValue();
    ...
    {
      final byte[] data = Eip712Encoder.encodeValue(domain.getVerifyingContract()).getValue();
  ...
  public void testEncodeDomainData() {
    final byte[] data = Eip712Encoder.encodeValue(domain.intoEip712Struct()).getValue();
  ...
  public void testTypedDataToSignedBytes() {
    final byte[] data = Eip712Encoder.typedDataToSignedBytes(domain, message);
  ...
  //
  //        assertEquals("0x997cbce617f1c5aca3616ddc2122db218d726e220fa040451d893430d7382ce5",
  // Numeric.toHexString(result));
  //    }
}
