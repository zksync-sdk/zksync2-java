...
public class SelfTransferTest {
  ...
  public void SelfTransfer() throws IOException {
    ...
    TransactionReceipt receipt =
        EthereumProvider.load(wallet.getZksync(), web3j, manager, gasProvider)
            ...
            .transfer(
                Token.ETH,
                Convert.toWei("0.117649", Convert.Unit.ETHER).toBigInteger(),
  ...
  public void deployTestContractWithConstructor() throws Exception {
    ...
    TransactionReceipt receipt =
        wallet
            .deploy(Numeric.hexStringToByteArray(bytecode), Numeric.hexStringToByteArray(calldata))
            .send();
  ...
  public void executeContract() throws Exception {
    ...
    Uint256 amount =
        new Uint256(new BigInteger("1234").multiply(new BigInteger(String.valueOf(10)).pow(18)));
    String address = credentials.getAddress();
    Function contractFunction =
        new Function("mint", Arrays.asList(new Address(address), amount), Collections.emptyList());

    TransactionReceipt receipt =
        wallet.execute("0x2da10A1e27bF85cEdD8FFb1AbBe97e53391C0295", contractFunction, null).send();
    System.out.println(receipt);
  }
  ...
  public void callTest() throws Exception {
    ...
    org.web3j.protocol.core.methods.request.Transaction call =
        org.web3j.protocol.core.methods.request.Transaction.createEthCallTransaction(
            null, contractAddress, FunctionEncoder.encode(contractFunction));

    EthCall ethCall = wallet.getZksync().ethCall(call, ZkBlockParameterName.COMMITTED).send();
    String fooName =
        (String)
            FunctionReturnDecoder.decode(ethCall.getValue(), contractFunction.getOutputParameters())
                .get(0)
                .getValue();
