...
public class IntegrationZkSyncWeb3RpcTest extends BaseIntegrationEnv {
        ...
        l1Web3
            .ethSendTransaction(
                Transaction.createEtherTransaction(
                    ...
                    Convert.toWei("1000000", Unit.ETHER).toBigInteger()))
            .sendAsync()
            .join();
  ...
  public void testGetBalanceOfNativeL1() throws IOException {
    EthGetBalance getBalance =
        l1Web3.ethGetBalance(credentials.getAddress(), DefaultBlockParameterName.LATEST).send();

    assertResponse(getBalance);
    System.out.printf(
        "%s: %d\n", credentials.getAddress(), Numeric.toBigInt(getBalance.getResult()));
    assertTrue(
        getBalance.getBalance().compareTo(Convert.toWei("0.2", Unit.ETHER).toBigInteger()) >= 0);
  }
  ...
  public void testDeposit() throws IOException {
    ...
    TransactionManager manager =
        new RawTransactionManager(l1Web3, credentials, chainId.longValue());
    ...
    TransactionReceipt receipt =
        EthereumProvider.load(zksync, l1Web3, manager, gasProvider)
            .join()
            .deposit(
                ETH,
  ...
  public void testDepositToken() throws IOException {
    Token token =
        zksync.zksGetConfirmedTokens(0, (short) 100).send().getResult().stream()
    ...
    TransactionReceipt approveReceipt =
        provider.approveDeposits(token, Optional.of(token.toBigInteger(10000000000L))).join();
    System.out.println(approveReceipt);

    TransactionReceipt receipt =
        provider
            .deposit(
                token, token.toBigInteger(10000000000L), BigInteger.ZERO, credentials.getAddress())
            .join();
  ...
  public void testGetBalanceOfNative() throws IOException {
    EthGetBalance getBalance =
        zksync.ethGetBalance(credentials.getAddress(), DefaultBlockParameterName.LATEST).send();
  ...
  public void testGetDeploymentNonce() throws Exception {
    NonceHolder nonceHolder =
        NonceHolder.load(
            ...
            zksync,
            new ReadonlyTransactionManager(zksync, credentials.getAddress()),
    ...
    TransactionReceipt receipt =
        zksync
            .ethGetTransactionReceipt(
                "0xea87f073bbb8826edf51abbbc77b5812848c92bfb8a825f82a74586ad3553309")
            .send()
            .getResult();
  ...
  public void testGetTransaction() throws IOException {
    org.web3j.protocol.core.methods.response.Transaction receipt =
        zksync
            .ethGetTransactionByHash(
                "0x60c05fffdfca5ffb5884f8dd0a80268f16ef768c71f6e173ed1fb58a50790e29")
            .send()
            .getResult();
  ...
  public void testTransferNativeToSelf() throws IOException, TransactionException {
    ...
    BigInteger nonce =
        zksync
            .ethGetTransactionCount(credentials.getAddress(), ZkBlockParameterName.COMMITTED)
            .send()
            .getTransactionCount();

    io.zksync.methods.request.Transaction estimate =
        io.zksync.methods.request.Transaction.createEtherTransaction(
            credentials.getAddress(),
            BigInteger.ZERO,
            BigInteger.ZERO,
            credentials.getAddress(),
  ...
  public void testTransferTokenToSelfWeb3jContract() throws Exception {
    ...
    TransactionReceipt receipt =
        erc20.transfer(credentials.getAddress(), token.toBigInteger(10)).send();
  ...
  public void testWithdrawToken() throws IOException, TransactionException {
    ...
    String l2Erc20Bridge =
        zksync.zksGetBridgeContracts().send().getResult().getL2Erc20DefaultBridge();
  ...
  public void testEstimateGas_Execute() throws IOException {
    Function transfer =
        ERC20.encodeTransfer("0xe1fab3efd74a77c23b426c302d96372140ff7d0c", BigInteger.valueOf(1L));
    String calldata = FunctionEncoder.encode(transfer);
    ...
    Executable execute =
        () -> {
          ...
          CounterContract contract =
              CounterContract.deploy(zksync, transactionManager, feeProvider).send();
        };
  ...
  public void testWeb3jContract() throws Exception {
    io.zksync.methods.request.Transaction deploy =
        io.zksync.methods.request.Transaction.createContractTransaction(
            credentials.getAddress(), BigInteger.ZERO, BigInteger.ZERO, CounterContract.BINARY);
    TransactionReceipt deployed = submitTransaction(deploy);
    CounterContract zkCounterContract =
  ...
  public void testDeployContract_Create() throws Exception {
    ...
    String precomputedAddress =
        ContractDeployer.computeL2CreateAddress(
                new Address(credentials.getAddress()), deploymentNonce)
            .getValue();
  ...
  public void testDeployContractWithDeps_Create() throws Exception {
    ...
    Transaction call =
        Transaction.createEthCallTransaction(
            null, contractAddress, FunctionEncoder.encode(getFooName));
    ...
    String fooName =
        (String)
            FunctionReturnDecoder.decode(ethCall.getValue(), getFooName.getOutputParameters())
                .get(0)
                .getValue();
  ...
  public void testDeployContractWithDeps_Create2() throws IOException, TransactionException {
    ...
    String precomputedAddress =
        ContractDeployer.computeL2Create2Address(
                new Address(credentials.getAddress()),
                Numeric.hexStringToByteArray(Import.BINARY),
                new byte[] {},
                salt)
            .getValue();
  ...
  public void testExecuteContract() throws IOException, TransactionException {
    ...
    BigInteger value =
        Numeric.toBigInt(zksync.ethCall(call, ZkBlockParameterName.COMMITTED).send().getValue());
  ...
  public void testGetTransactionDetails() throws IOException {
    ZksGetTransactionDetails response =
        zksync
            .zksGetTransactionDetails(
                "0x0898f4b225276625e1d5d2cc4dc5b7a1acb896daece7e46c8202a47da9a13a27")
            .send();
  ...
  public void testGetTransactionByHash() throws IOException {
    ZksGetTransactionByHash response =
        zksync
  ...
  public void testGetBlockByHash() throws IOException {
    ZksBlock response =
        zksync
            .zksGetBlockByHash(
                "0xd933be650bf97d92c591a88c993b702b368af5480adf019afddee7e8c1e5ce2e", true)
            .send();
  ...
  public void testGetBlockByNumber() throws IOException {
    ZksBlock response =
        zksync.zksGetBlockByNumber(DefaultBlockParameter.valueOf("0xb108e"), true).send();
